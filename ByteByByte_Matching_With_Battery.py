...
def receive_battery():
    try:
        ...
        if battery_percent is not None:
            print(f"🔋 Battery Remaining: {battery_percent}%")
            return jsonify({'status': 'success', 'battery': battery_percent}), 200
        else:
            return jsonify({'status': 'fail', 'message': 'No battery data'}), 400
